...
fn build_inner(
    ...
    let devdir = if host.contains("apple") {
        let mut xcode_select =
            hermetic_apple_command("/usr/bin/xcode-select", &base_env, &config_env);
        xcode_select.arg("-p");
        capture(&mut xcode_select, "xcode-select -p")
